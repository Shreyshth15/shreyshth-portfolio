...
import { ArrowUpRight, FileText, Mail, Linkedin, Phone, Send, Loader2 } from "lucide-react";

const API = `${process.env.REACT_APP_BACKEND_URL}/api`;

const links = [
  { key: "email", label: "Email", value: PROFILE.email, href: `mailto:${PROFILE.email}`, Icon: Mail, ext: false },
  { key: "linkedin", label: "LinkedIn", value: PROFILE.linkedinLabel, href: PROFILE.linkedin, Icon: Linkedin, ext: true },
  { key: "phone", label: "Phone", value: PROFILE.phone, href: PROFILE.phoneHref, Icon: Phone, ext: false },
];
      ...
      <div className="mx-auto max-w-[1400px]">
        <Reveal>
          ...
          <p className="mt-8 max-w-2xl text-lg leading-relaxed text-slate-300">
            Open to asset management, credit research and consulting roles, and to good conversations even when nothing's on the table. Drop a message below or reach me directly.
          </p>
        </Reveal>
          ...
          <Reveal delay={0.1}>
            ...
            >
              <span className="flex items-center gap-3">
                ...
                <span>
                  <span className="block font-display text-lg text-slate-50">View Resume</span>
            ...
            <div className="mt-4 space-y-px overflow-hidden rounded-2xl border border-white/10 bg-white/10">
              {links.map(({ key, label, value, href, Icon, ext }) => (
                <a
                  key={key}
                  ...
                  rel="noopener noreferrer"
                  className="group flex items-center justify-between bg-[#050b1c] p-5 transition-colors hover:bg-blue-500/[0.07] md:p-6"
                >
                  <span>
                    <span className="flex items-center gap-2 font-mono text-[11px] uppercase tracking-[0.18em] text-slate-500">
                      <Icon className="h-3.5 w-3.5" /> {label}
                    </span>
                    <span className="mt-1.5 block text-base text-slate-100">{value}</span>
                  </span>
                  <ArrowUpRight className="h-5 w-5 text-slate-500 transition-all group-hover:-translate-y-0.5 group-hover:translate-x-0.5 group-hover:text-blue-400" />
                </a>
              ))}
